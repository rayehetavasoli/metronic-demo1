...
import { InspectionCostFormProps, InspectionCost } from '@/types/main';
import "./style/InspectionCostForm.css";

const InspectionCostForm: React.FC<InspectionCostFormProps> = ({ inspectionCost, onClose, onSave, mode }) => {
  const [formData, setFormData] = useState<InspectionCost>({
    id: inspectionCost?.id || 0,
    inspectionType: inspectionCost?.inspectionType || '',
    cost: inspectionCost?.cost || 0,
  });
  ...
  const handleSubmit = () => {
    const { inspectionType , cost } = formData;
    if (!inspectionType || !cost ) {
      ...
      <CloseCircle className="w-6 h-6 relative justify-end items-start inline-flex" />
        ...
        <div className="flex-col justify-center items-start gap-5 inline-flex">
          <div className="self-stretch px-2 justify-start items-center gap-1 inline-flex">
            ...
            <div className="text-lg font-semibold dark:text-gray-100">{mode === 'add' ? 'افزودن بازرسی جدید' : 'ویرایش اطلاعات بازرسی'}</div>
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="flex flex-row gap-4 justify-center items-center">
            <div className="flex-col justify-center items-end gap-7 inline-flex">
              <label className="label dark:text-gray-100">نوع بازرسی</label>
              <label className="label dark:text-gray-100">هزینه بازرسی</label>
            </div>

            <div className="p-3 flex-col justify-center items-center gap-7 inline-flex"> 
              {['inspectionType', 'cost'].map((field, index) => (
                <div key={index} className="bg-light-bg-primary dark:bg-dark-bg-primary rounded-xl justify-between items-center gap-2 inline-flex w-full px-3 py-2">
                  <input
                    ...
                    name={field}
                    placeholder={`${
                      field === 'inspectionType' ? 'نوع بازرسی' : 
                      'هزینه بازرسی'
                    }`}
                    value={formData[field as keyof InspectionCost]}
...
export default InspectionCostForm;
