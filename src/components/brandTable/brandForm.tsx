...
const BrandForm: React.FC<BrandFormProps> = ({ brand, onClose, onSave, mode }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50" dir="rtl">
      <div className="p-5 bg-white dark:bg-black rounded-[32px] shadow flex-col justify-center items-end inline-flex gap-3">
      ...
      <CloseCircle className="w-6 h-6 relative justify-end items-start inline-flex" />
        ...
        <div className="flex-col justify-center items-start gap-5 inline-flex">
          <div className="self-stretch px-2 justify-start items-center gap-1 inline-flex">
            {mode === 'add' ? <AddCircle className="w-5 h-5 text-gray-900 dark:text-gray-100" /> : <Edit2 className="w-5 h-5 text-gray-900 dark:text-gray-100" />}
            <div className="text-lg font-semibold dark:text-gray-200">{mode === 'add' ? 'افزودن برند جدید' : 'ویرایش اطلاعات برند'}</div>
          </div>
              ...
              {['brandName', 'founded', 'country', 'industry'].map((field, index) => (
                <div key={index} className="w-full px-3 py-2 bg-light-bg-primary dark:bg-dark-bg-primary rounded-xl justify-between items-center gap-2 inline-flex">
                  <input
                    ...
                    className="grow shrink basis-0 text-right text-black dark:text-gray-100 text-md font-normal bg-light-bg-primary  dark:bg-dark-bg-primary w-full border-none outline-none focus:ring-0 "
                  />
                </div>
              ))}
            </div>
